	...
	xdef	_ti92plus	; Create Ti-89 program
	xdef	_v200		; Create V200 program

	xdef	_exit		; there is an exit point
	DEFINE	_version02	; Define version 02
...
;	a0.l = title of the box
	...
	moveq	#1,d4		;
	bsr	fill		; Fill in WHITE

	move.w	d2,d4		; Copy Width
	move.w	d3,d5		; Copy Hight
	addq.w	#1,d4
	addq.w	#1,d5
	...
	move.w	d4,d2
	moveq	#8,d3
	moveq	#2,d4		; Fill in BLACK
	bsr	fill		; The title menu
	...
	jsr	tios::FontSetSys
	addq.l	#2,a7
	move.w	d0,d3		; Save previous font
	jsr	tios::DrawStrXY	; Display title
	move.w	d3,(a7)		; Restore previous font
	jsr	tios::FontSetSys
	lea	10(a7),a7
...
clear_dialog:
	movem.l	d0-d4,-(a7)
	movem.w	dialogpos(pc),d0-d3	; Previous Dialog Struct
	moveq	#1,d4			; White
	bsr	fill
	movem.l	(a7)+,d0-d4
	rts
...
show_dialog:
	...
	move.w	(a6)+,d0	; Read X1
	move.w	(a6)+,d1	; Read Y1
	move.w	(a6)+,d2	; Read X2
	move.w	(a6)+,d3	; Read Y2
	sub.w	d0,d2		; Compute Width
	sub.w	d1,d3		; Compute Height
	lea	dialogpos(pc),a2
	movem.w	d0-d3,(a2)	; Save Dialog to be erased latter
	...
	move.w	#2,-(a7)	; Set Font
	jsr	tios::FontSetSys
	move.w	d0,d7		; Save previous font
	movem.w	(a2),d0-d1	; Reget X and Y
	add.w	(a6)+,d0	; Get absolute coordinates
	add.w	(a6)+,d1
	addq.w	#4-2,(a7)	; From 2 to 4
	pea	(a6)		; Push String
	move.w	d1,-(a7)
	move.w	d0,-(a7)
	jsr	tios::DrawStrXY
	move.w	d7,(a7)		; Old Font
	jsr	tios::FontSetSys
	lea	10(a7),a7
	movem.l	(a7)+,d0-d7/a0-a6
	rts
...
erase_rect:
	movem.l	d0-d4/a0,-(a7)
	lea	7*4(a7),a0
	move.w	(a0)+,d0	; Read X1
	move.w	(a0)+,d1	; Read Y1
	move.w	(a0)+,d2	; Read X2
	move.w	(a0),d3		; Read Y2
	sub.w	d0,d2		; Compute Width
	sub.w	d1,d3		; Compute Height
	moveq	#1,d4		; In white
	bsr	fill
...
gray4:	movem.l d1-d7/a0-a6,-(a7)
	...
	subq.b	#1,d4
	beq.s	\hw1
		add.l	#$F00,(a7)		; Un en plus pour HW2
\hw1	jsr	tios::HeapAllocHigh
	addq.l	#4,a7
	move.w	d0,handle
	beq	graylib_fail
...
gray2:
	movem.l d1-d7/a0-a6,-(a7)
        move.w  handle(pc),-(a7)
	beq.s	\fail
		move.l	oldint1(pc),GHOST_SPACE+$64
...
gray7:
	...
	subq.b	#1,d4
	beq.s	\hw1
		add.l	#$F00,(a7)		; Un en plus pour HW2, 3
\hw1	jsr	tios::HeapAllocHigh
	addq.l	#4,a7
	move.w	d0,handle
	beq.s	graylib_fail
...
\loop			move.l	(a0)+,(a2)+
			dbf	d0,\loop
		; Set it for Tios drawing
		move.l	#239*65536+127,-(a7)	
		pea	(a1)
		jsr	tios::PortSet
		addq.l	#8,a7
		; Set up the auto-int
		move.l	#int1HW2,GHOST_SPACE+$64
\done	move.w	(a7)+,d0
	rts
...
\test_fs	
	...
	move.w	phase2(pc),d0	; d0 = phase2
	bne.s	\no_test_fs
		; Attend la frame synchro
		move.b	Current_FS(pc),d2
		move.b	($70001D),d3
		eor.b	d3,d2
		...
		move.w	#1280*2,d1	; Offset pour p2 3
\no_test_fs:
	...
	bne.s	\no_swap_screen
		...
		bgt.s	\no
			move.b	ResetPhase(Pc),d4	; Reinit Screen Phase
\no:		move.b	d4,(a0)		; Save Screen phase
\no_swap_screen:

	subq.w	#1,d0
	bne.s	\nop2			
		move.w	#1280,d1	; Offset par rapport au debut pour l'affichage en p2 2
\nop2	move.w	d0,phase2		; Sauvegarde p2
	
	; Copie du tiers de l'ecran
	; Cacul de l'adresse source
	moveq	#0,d2
	move.b	phase(pc,d4.w),d2
	cmp.b	phase+1(Pc,d4.w),d2
	beq	\stop			; C encore le meme plan => C inutile de le recopier

	move.l	plane0(Pc,d2.w),a0
	adda.w	d1,a0
	...
	movem.l (a0)+,d0-d7/a2-a6
	movem.l d0-d7/a2-a6,(18*13*4)(a1)
	movem.l (a0)+,d0-d7/a2-a6
	...
	movem.l (a0)+,d0-d7/a2-a6
	movem.l d0-d7/a2-a6,(23*13*4)(a1)	; 1248 octets copies

	movem.l (a0)+,d0-d7
	movem.l d0-d7,(24*13*4)(a1)		; +32 = 1280 octets copies
